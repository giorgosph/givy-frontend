...
import { validateAllInputs, emailRegex, lettersRegex, numbersRegex, passwordValidation, required, usernameValidation, validateSpaces } from '../../utils/formValidations';
...
const Signup = (props: PropsType) => {
  ...
  const { control, handleSubmit } = useForm<SignupFormType>({ mode: 'onChange' });
  ...
  return (
    <KeyboardAvoidingView behavior={isAndroid ? 'padding' : 'height'}>
      <KeyboardAwareScrollView enableOnAndroid={true} style={styles.container}>
        ...
        <CustomInput control={control} name="address2" title="address 2" rules={validateAllInputs} autoComplete={AC.addressLine2} />
        <CustomInput control={control} name="postalCode" title="postal code" rules={validateAllInputs} autoComplete={AC.postalCode} />
